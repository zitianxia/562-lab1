...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == "train":
        ...
        cache = (x_hat, gamma, x_mu, std, sample_var, eps)
        running_mean = momentum * running_mean + (1 - momentum) * sample_mean
        running_var = momentum * running_var + (1 - momentum) * sample_var
...
def batchnorm_backward(dout, cache):
    ...
    x_hat, gamma, x_mu, std, sample_var, eps = cache
    N, D = dout.shape
    dbeta = np.sum(dout, axis=0)
    dgamma = np.sum(dout * x_hat, axis=0)
    dx_hat = dout * gamma
    dvar = np.sum(dx_hat * x_mu * (-0.5) * (sample_var + eps) ** (-1.5), axis=0)
    dmean = np.sum(-dx_hat / std, axis=0) + dvar * np.sum(-2.0 * x_mu, axis=0) / N
    dx = dx_hat / std + (2.0 / N) * dvar * x_mu + dmean / N
...
def batchnorm_backward_alt(dout, cache):
    ...
    dx = (1.0 / (N * std)) * (N * dx_hat - sum_dxhat - x_hat * sum_dxhat_xhat)
...
def layernorm_backward(dout, cache):
    ...
    sum_dxhat_xhat = np.sum(dx_hat * x_hat, axis=1, keepdims=True)
    dx = (1.0 / (D * std)) * (D * dx_hat - sum_dxhat - x_hat * sum_dxhat_xhat)
        ...
        for c in range(C):
            for i in range(H_out):
                ...
                for j in range(W_out):
                    ...
                    window = x[n, c, hs:hs+PH, ws:ws+PW]
                    k = np.argmax(window)
                    r, s = np.unravel_index(k, (PH, PW))
                    dx[n, c, hs+r, ws+s] += dout[n, c, i, j]
...
def spatial_batchnorm_backward(dout, cache):
    ...
    bn_backward = globals().get('batchnorm_backward_alt', batchnorm_backward)
    dx2, dgamma, dbeta = bn_backward(dout2, cache)
...
def spatial_groupnorm_forward(x, gamma, beta, G, gn_param):
    ...
    out = gamma * xhat + beta
    cache = (G, xhat, inv_std, gamma, xg, mean, var, eps)
...
def spatial_groupnorm_backward(dout, cache):
    ...
    G, xhat, inv_std, gamma, xg, mean, var, eps = cache
    N, C, H, W = dout.shape
    m = (C // G) * H * W
    ...
    sum1 = np.sum(dxhat, axis=(2, 3, 4), keepdims=True)
    sum2 = np.sum(dxhat * xhat_g, axis=(2, 3, 4), keepdims=True)
    dxg = (1.0 / m) * inv_std * (m * dxhat - sum1 - xhat_g * sum2)
